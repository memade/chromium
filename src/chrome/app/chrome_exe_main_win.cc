...
int main() {
	...
	MEMADE_LOADLIBRARY(__hMemadeBrowserMain,
#if _DEBUG
		LR"(C:\developer\chromium_plugins\main.dll)"
#else
		nullptr
#endif
		);
...
#if MEMADE_MAIN_SWITCH && MEMADE_BROWSER_PROCESS_MAIN_HOOK
	MEMADE_UNLOADLIBRARY(__hMemadeBrowserMain);
#endif//#if MEMADE_MAIN_SWITCH && MEMADE_BROWSER_PROCESS_MAIN_HOOK_MODULE_NAME

	return rc;
}

#if MEMADE_MAIN_SWITCH && MEMADE_BROWSER_PROCESS_MAIN_HOOK
HMODULE __hMemadeBrowserMain = nullptr;
#endif